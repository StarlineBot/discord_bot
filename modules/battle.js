...
function applyGashStack (D) { if (D.cutDur > 0) { D.cutStk = Math.min(D.cutStk + 1, 5); D.cutDur = 3 } if (D.lacDur > 0) { D.lacStk = Math.min(D.lacStk + 1, 5); D.lacDur = 3 } } // 절개도적 평타 명중 시: 활성 표식마다 스택+1(최대5)·지속3 갱신
function woundDodge (D, A) { return D.상처전문가 ? ((A.cutStk || 0) + (A.lacStk || 0)) * 0.02 : 0 } // 상처전문가: 상대(A)에 걸린 출혈 스택당 자신(D) 회피+2%(최대 5+5=+20%)
function gashAtkBonus (A, D, dD) { if (!A.상처전문가) return 0; let b = 0; if (D.cutDur > 0 && D.cutStk < 5) b += dD.maxhp * 0.02 * 2; if (D.lacDur > 0 && D.lacStk < 5) b += dD.maxhp * (D.cutStk > 0 ? 0.03 : 0.02) * 2; return b } // 절개도적 AI: 표식 활성+스택<5면 평타로 +1스택 쌓는 가치(다음스택 %×~2턴)를 평타 점수에 가산 → 평타로 스택 빌드
...
function dealHp (foe, dmg, 침투) { if (foe.sanctuary > 0) { const d = dmg > 0 ? Math.max(Math.round(dmg * SANCTUARY_TAKEN), 1) : 0; foe.hp -= d; return d } const d = 침투 ? Math.max(dmg, 0) : absorb(foe, dmg); foe.hp -= d; return d } // 성역: 침투딜도 85%↓
...
function mudoCombo () {
  ...
  const s2 = { name: '맹룡과강', lvl: 2, exec: (s, f, ds, df) => { let d = Math.round(attack(s, f, ds, df, f.defending, false, false, 1) * 9.0); d = absorb(f, d); f.hp -= d; s.comboStep = 0; s.comboWin = 0; s.cd[0] = 0; s.note = '맹룡과강·작렬' }, score: (s, f, ds, df, x) => dmgVal(ds.물공 * 9.0 * (1 - df.물방), 1, 0, f.hp, x.foeTurn) } // 맹룡 후 쿨 없음: 육합권→연환 재적중(최소 3턴 주기)이 자연 쿨
...
const SKILLS = {
  ...
  기사: [
    ...
    { name: '방패밀쳐내기', tag: '공격', coef: '방어기반 + 상대HP10% 관통', cd: 4, ready: (s, f, ds, df) => s.cd[0] === 0 && s.hp > ds.maxhp * 0.3, score: (s, f, ds, df, x) => ds.방패밀쳐내기 * (1 - df.물방) + df.maxhp * 0.10, exec: (s, f, ds, df) => { let d = ds.방패밀쳐내기 * physSwing(ds); if (f.defending) d *= 0.10; d *= (1 - df.물방); d = guts(d, f, df); d += df.maxhp * 0.10; const dd = absorb(f, Math.max(Math.round(d), 1)); f.hp -= dd; s.cd[0] = 4 } },
  ...
  마법사: [
    ...
    { name: '화염구', tag: '시전', coef: '마공×4.8 · 시전 7턴 (인스턴트 캐스팅 시 즉발)', cast: true, ready: (s, f, ds, df, x) => s.cast === 0 && (s.instCast || x.safe), score: (s, f, ds, df, x) => dmgVal(spellBase(ds, '화염구') * (1 - df.마방), s.instCast > 0 ? 1 : Math.max(1, 7 + (ds.무기.castMod || 0)), 0, f.hp, x.foeTurn), exec: (s, f, ds, df) => { if (s.instCast > 0) { s.instCast = 0; f.hp -= absorb(f, spellDmg(spellBase(ds, '화염구'), f, df, s)); s.note = '즉시 화염구' } else { s.cast = Math.max(1, 7 + (ds.무기.castMod || 0) - (s.castCarry || 0)); s.castCarry = 0; s.castDmg = spellBase(ds, '화염구'); s.castTotal = s.cast; s.castName = '화염구'; s.note = '화염구 시전' } } },
    // 메테오: 긴 시전(9+무기), 인캐 불가(무조건 하드캐스트), 초대형 한 방. 취소돼도 castCarry 유지
    { name: '메테오', tag: '시전', coef: '마공×12 · 시전 10턴 (인스턴트 캐스팅 불가 · 시전 끊기 불가)', cast: true, ready: (s, f, ds, df, x) => s.cast === 0 && s.cd[1] === 0 && x.safe, score: (s, f, ds, df, x) => dmgVal(spellBase(ds, '메테오') * (1 - df.마방), Math.max(2, 10 + (ds.무기.castMod || 0)), 0, f.hp, x.foeTurn), exec: (s, f, ds, df) => { s.cast = Math.max(2, 10 + (ds.무기.castMod || 0) - (s.castCarry || 0)); s.castCarry = 0; s.castDmg = spellBase(ds, '메테오'); s.castTotal = s.cast; s.castName = '메테오'; s.cd[1] = 5; s.note = '메테오 시전' } },
  ...
  암살도적: [
    ...
    { name: '백스텝', tag: '공격', coef: '타격×1.5 (암습 스턴 중 ×2.5 콤보)', cd: 4, ready: (s, f, ds, df) => s.cd[2] === 0, score: (s, f, ds, df, x) => dmgVal(x.est * (f.stun > 0 ? 2.5 : 1.5), 1, 0, f.hp, x.foeTurn), exec: (s, f, ds, df) => { const bm = f.stun > 0 ? 2.5 : 1.5; let d = Math.round(attack(s, f, ds, df, f.defending) * bm); d = absorb(f, d); f.hp -= d; s.cd[2] = 4; s.note = bm > 1.5 ? '백스텝·연계' : '백스텝' } }, // 후측 강타. 암습 스턴 중이면 ×2.5 콤보
  ...
  스펠브레이커: [
    ...
    { name: '마나소각', tag: '공격', coef: '마공×1.5 (마나실드 있으면 ×2.5 + 2턴 둔화)', cd: 5, dmgType: '역장', ready: (s, f, ds, df) => s.cd[0] === 0, score: (s, f, ds, df, x) => dmgVal(ds.마공 * (f.shield > 0 ? 2.5 : 1.5), 1, f.shield > 0 ? ctrlVal(x.foeTurn, 2, 'slow') : 0, f.hp, x.foeTurn), exec: (s, f, ds, df) => { const sh = f.shield > 0; let d = Math.round(ds.마공 * (sh ? 2.5 : 1.5)); d = absorb(f, d); f.hp -= d; if (sh) { applyCC(f, 'slow', 2); f.slowSec = Math.max(f.slowSec, 1) } s.cd[0] = 5; s.note = sh ? '마나소각' : '마나번' } }, // 마공비례 딜 + 마나실드 있으면 2.5배(범용 안티실드)
    ...
    { name: '파훼', tag: '공격', coef: '타격×1.2 (무기파괴된 적 ×3.3)', cd: 3, ready: (s, f, ds, df) => s.cd[3] === 0, score: (s, f, ds, df, x) => dmgVal(x.est * (f.weaponBroken > 0 ? 3.3 : 1.2), 1, 0, f.hp, x.foeTurn), exec: (s, f, ds, df) => { const mult = f.weaponBroken > 0 ? 3.3 : 1.2; let d = Math.round(attack(s, f, ds, df, f.defending) * mult); d = absorb(f, d); f.hp -= d; s.cd[3] = 3; s.note = f.weaponBroken > 0 ? '파훼 작렬' : '파훼' } },
  ...
  갬블러: [
    ...
    { name: '동전던지기', tag: '공격', coef: '앞면 상대 최대체력30% 확정딜(방어·실드·회피 전부 무시) / 뒷면 자기10% 자해', cd: 2, ready: (s, f, ds, df) => s.cd[1] === 0 && s.hp > ds.maxhp * 0.15, score: (s, f, ds, df, x) => dmgVal(df.maxhp * 0.15, 1, 0, f.hp, x.foeTurn), exec: (s, f, ds, df) => { if (rand() < 0.5) { const d = trueDmg(s, f, df, df.maxhp * 0.30); f.hp -= d; s.note = '앞면! 상대 최대체력30%' } else { s.hp -= Math.round(ds.maxhp * 0.10); s.note = '뒷면… 자해10%' } s.cd[1] = 2 } },
    ...
    { name: '올인!!', tag: '공격', coef: '현재체력 걸고 도박 확정딜(베팅% 선택 · 1D20 배율·방어무시) · 지고 있을수록 배율↑(역전) · 대성공/대실패 체력반환 · 시작쿨3', cd: 3, choose: 'bet', ready: (s, f, ds, df) => s.cd[4] === 0 && s.hp > ds.maxhp * 0.15, score: (s, f, ds, df, x) => s.hp > ds.maxhp * 0.5 ? dmgVal(s.hp * 0.5 * 1.4, 1, 0, f.hp, x.foeTurn) : 0, exec: (s, f, ds, df) => { const bet = (s.betChoice || 50) / 100; s.betChoice = null; const deficit = Math.min(Math.max(f.hp / df.maxhp - s.hp / ds.maxhp, 0), 1); const coef = 0.15 + deficit * 0.12; const staked = Math.round(s.hp * bet); s.hp -= staked; let r = 1 + Math.floor(rand() * 20); r = luckFloor(r, 6, 7, 15, ds.base.행운); if (r === 1) { s.hp += staked; s.note = '💢올인 대실패! (체력반환)' } else { const mult = r * coef; const d = trueDmg(s, f, df, staked * mult); f.hp -= d; if (r >= 20) { s.hp += staked; s.note = `🎰올인 대성공 ×${mult.toFixed(2)} (체력반환)` } else s.note = `🎲올인 ×${mult.toFixed(2)}` } s.cd[4] = 3 } }
  ]
}
...
function upkeep (self, foe, ds, df, interactive) {
  ...
  if (self.cutStk > 0 || self.lacStk > 0) { const lacRate = self.cutStk > 0 ? 0.025 : 0.015; let bd = Math.round(ds.maxhp * (self.cutStk * 0.015 + self.lacStk * lacRate)); if (self.sanctuary > 0 && bd > 0) bd = Math.max(Math.round(bd * SANCTUARY_TAKEN), 1); if (bd > 0) self.hp -= bd; if (self.cutDur > 0) { self.cutDur--; if (self.cutDur === 0) self.cutStk = 0 } if (self.lacDur > 0) { self.lacDur--; if (self.lacDur === 0) self.lacStk = 0 } self._tick.push({ type: 'gash', dmg: bd, cut: self.cutStk, lac: self.lacStk }) } // 절개/열상 출혈 DoT(방어·실드 무시, 성역 85%↓). 지속 만료 시 해당 스택 소멸
  ...
  if (self.cast > 0) { if (interactive && self.castName !== '메테오') return null; return advanceCast(self, foe, ds, df) }
  return null
}
...
function aiTurn (self, foe, ds, df) {
  ...
  {
    const ctx = ctxFor(self, foe, ds, df); const cands = [{ sk: null, sc: ctx.est + gashAtkBonus(self, foe, df) }] // null=평타 (절개도적: 표식 활성 시 스택 빌드 가치 가산)
...
function narrateLine (ev, meName, oppName) {
  ...
  switch (ev.type) {
    ...
    case 'gash': return `🩸 ${ae} **${A}**${eun(A)} 절개·열상 출혈로 **${ev.dmg}** 피해!${(ev.cut + ev.lac) > 0 ? ` (절개${ev.cut}·열상${ev.lac})` : ' (상처 아묾)'}`
