...
const CHARS = {
  ...
  기사: { emoji: '🛡', 힘: 60, 지능: 10, 체력: 100, 민첩: 35, 솜씨: 68, 행운: 30, 무기: '검방', id: '순수 탱커' },
  ...
  세이지: { emoji: '📖', 힘: 60, 지능: 65, 체력: 55, 민첩: 50, 솜씨: 50, 행운: 30, 무기: '마도서', id: '혼합 하이브리드' },
...
const WEAPONS = {
  ...
  단검: { 계열: '물리', 평타: '물리', range: '근접', hands: 1, 기본공: 0.78, tempoMul: 1.0, 크리보너스: 0.15, 회피보너스: 0.15, 방어: '패링' },
  ...
  활: { 계열: '물리', 평타: '물리', range: '원거리', hands: 2, 기본공: 1.05, tempoMul: 1.0, 원거리페널티: 0.3, 방어: '회피' },
  ...
  마도서: { 계열: '마법', 평타: '혼합', range: '원거리', hands: 2, 방어: '무기막기' },
  검오브: { 계열: '마법', 평타: '융합', range: '근접', hands: 1, 방어: '무기막기' }
}
// 무기 표시 라벨(이모지 + 이름)
const WEAPON_LABEL = {
  검방: '🛡️검/방', 단검: '🗡단검', 쌍검: '🗡🗡쌍검', 양검: '⚔️양손검', 양둔: '🔨양손둔기',
  양도끼: '🪓양손도끼', 활: '🏹활', 스태프: '🔮스태프', 완드: '✨완드', 마도서: '📖마도서', 검오브: '⚡검/오브'
}
...
function derive (c) {
  ...
  return {
    ...
    물크기본: 2.5, 마크기본: 1.5, 크랜폭: step(c.행운, 0.15) * 2, 비껴무효: step(c.행운, 3) / 100,
...
function attack (A, D, dA, dD, defending, guaranteed) {
  ...
  if (dA.hybrid) {
    ...
    if (!evaded) {
      let block = 0
      if (dD.방패막기 && rand() < dD.방패막기) { block = 0.30; 고정 = dD.방패고정; A.lastDef = '방패막기' }
  ...
  if (dA.fusion) {
    if (!guaranteed && luckRoll(rand() < dD.물회, lsD, (D.luckLock > 0 ? 0 : dD.리롤))) { A.lastDef = '회피'; if (dD.sigEcho) D.echoReady = 1; return 0 }
    let d = dA.물공 + dA.마공; 고정 = 0
    if (dD.방패막기 && rand() < dD.방패막기) { d *= 0.30; 고정 = dD.방패고정; A.lastDef = '방패막기' } else if (dD.무기막기 && rand() < dD.무기막기) { d *= 0.50; A.lastDef = '무기막기' }
  ...
  if (!magic) {
    ...
    if (luckRoll(rand() < dD.물회, lsD, (D.luckLock > 0 ? 0 : dD.리롤))) { A.lastDef = '회피'; if (dD.sigEcho) D.echoReady = 1; return 0 }
    const hits = dA.무기.다단 || 1          // 쌍검: 2연타(기본공에 발당 배수 반영)
    let block = 0, wblk = 1
    if (dD.방패막기 && rand() < dD.방패막기) { block = 0.30; 고정 = dD.방패고정; A.lastDef = '방패막기' } else if (dD.무기막기 && rand() < dD.무기막기) { wblk = 0.50; A.lastDef = '무기막기' }
    ...
    for (let i = 0; i < hits; i++) {
      ...
      let crit = luckRoll(rand() < (dA.크리 + (A.luckBuff > 0 ? 0.2 : 0)), ls, (A.luckLock > 0 ? 0 : dA.리롤) + (A.luckBuff > 0 ? 0.2 : 0))
      if (A.echoReady) { crit = true; A.echoReady = 0 }
      if (crit) { h *= (dA.물크기본 + rand() * dA.크랜폭) * (A.luckBuff > 0 ? 2 : 1); A.lastCrit = true }
      dmg += h
    }
    ...
    let mBlock = 0
    if (dD.방패막기 && rand() < dD.방패막기) { mBlock = 0.30; 고정 = dD.방패고정; A.lastDef = '방패막기' }
    dmg = 0; const braw = []
    for (let i = 0; i < bolts; i++) {
      let b = dA.마공 * dA.평타계수 * dA.마뎀너프 * per   // 평타계수: 스태프 약한평타 / 마뎀너프: 완드
      if (mBlock) b *= 0.30
    ...
    if (bolts > 1) { A.lastBolt = true; A._braw = braw }   // 연쇄볼트 발당 내역(최종에서 비례 배분)
  }
  dmg -= 고정
  ...
  if (A._braw && A._braw.length > 1) { const s = A._braw.reduce((a, b) => a + b, 0) || 1; A.boltHits = A._braw.map(r => Math.round(r / s * fin)) }
  if (A.tLeech) A.hp = Math.min(dA.maxhp, A.hp + Math.round(dA.base.체력 / 2))
  if (D.tThorns) A.hp -= Math.max(Math.round(fin * 0.05), 1)
  return fin
}
...
const SKILLS = {
  전사: [
    ...
    { name: '방어파괴', ready: (s, f, ds, df) => s.cd[0] === 0 && f.hp > df.maxhp * 0.3, score: (s, f, ds, df, x) => x.est * 1.2 + df.maxhp * 0.06, exec: (s, f, ds, df) => { f.sunder = 2; const d = attack(s, f, ds, df, f.defending); f.hp -= d + ds.힘절반 + Math.round(df.maxhp * 0.06); s.cd[0] = 3 } },
  ...
  기사: [
    ...
    { name: '방패가격', ready: (s, f, ds, df) => s.cd[0] === 0 && s.hp > ds.maxhp * 0.3, score: (s, f, ds, df, x) => ds.방패가격 * (1 - df.물방) + df.maxhp * 0.08, exec: (s, f, ds, df) => { let d = ds.방패가격 * physSwing(ds); if (f.defending) d *= 0.10; d *= (1 - df.물방); d = guts(d, f, df); d += df.maxhp * 0.08; f.hp -= Math.max(Math.round(d), 1); s.cd[0] = 3 } },
  ...
  마법사: [
    // 시전턴 = 기본 6 + 무기 castMod(스태프 -2 → 4). 캐스팅 이점이 무기에서 나옴
    { name: '파이어볼', ready: (s, f, ds, df, x) => s.cast === 0 && x.safe, score: (s, f, ds, df, x) => ds.메테오, exec: (s, f, ds, df) => { s.cast = Math.max(1, 6 + (ds.무기.castMod || 0)); s.note = '파이어볼 시전' } },
    // 인스턴트 캐스팅: 시전시간 0 = 즉시 파이어볼 발사. 시작 3쿨(오프닝 봉인)
    { name: '인스턴트캐스팅', ready: (s, f, ds, df) => s.cd[1] === 0 && s.cast === 0, score: (s, f, ds, df, x) => ds.메테오, exec: (s, f, ds, df) => { f.hp -= Math.round(guts(ds.메테오 * 0.7, f, df) * magicGraze()); s.cd[1] = 5; s.note = '파이어볼 즉시시전' } }
  ],
  원드마법사: [
...
const SKILL_CD = { 방어파괴: 3, 돌진: 5, 광폭화: '버프', 재생의광기: 5, 방패가격: 3, 도발: 5, 파이어볼: '시전', 인스턴트캐스팅: 5, 암습: 10, 처형: 5, 약점간파: 4, 견제사격: 4, 약점봉인: 3, 중력베기: 5, 오토스펠: 3, 연환주문: 4, 행운폭발: '버프', 동전던지기: 2, 연쇄주문: '버프', 마력충전: 5 }
...
function narrateLine (ev, meName, oppName) {
  ...
  const hurtBd = () => {
    ...
    if (ev.boltHits && ev.boltHits.length > 1) return `${te} **${T}**${eun(T)} 볼트 **${ev.boltHits.join('** + **')}** (총 **${ev.dmg}**)의 피해를 입었다.`
    return hurt(ev.dmg)
  }
  ...
  switch (ev.type) {
    case 'attack':
      ...
      if (ev.crit) return `💥 ${ae} **${A}**의 ${ev.ph != null ? '혼합 ' : ''}공격이 치명타로 적중! ${boltTag}${hurtBd()}${defTag}`
      return `${ev.ph != null ? '⚔️' : (ev.bolt ? '✨' : '⚔️')} ${ae} **${A}**의 ${ev.ph != null ? '혼합 공격' : '공격'}! ${boltTag}${hurtBd()}${defTag}`
    case 'skill': {
